...
class GridFFN:
    ...
    def _build_grid(self):
        input_layer = self._clique_data.shape[1]

        class_weights = [{0: 1. / (self._vertices - self._clique_size), 1: 1. / self._clique_size},
                         {0: 3. / int(self._vertices - self._clique_size), 1: 1. / self._clique_size}]
        epochs = [10, 100, 1000]
        ...
        layers = [[input_layer, 1], [input_layer, 50, 30, 1], [input_layer, 100, 20, 1]]
    ...
    def run_models(self):
        ...
        ffn = FFNCliqueDetector(self._vertices, self._probability, self._clique_size, self._is_directed,
                                use_motifs=True, use_extra=True)
        clique_matrix = ffn.clique_matrix
        non_clique_matrix = ffn.non_clique_matrix
        scaler = StandardScaler()
        scaler.fit(ffn.matrix)
        additional_clique, additional_non_clique = ffn.additional_features
        self._clique_data = np.hstack((scaler.transform(clique_matrix), additional_clique))
        self._non_clique_data = np.hstack((scaler.transform(non_clique_matrix), additional_non_clique))
        ...
        w.writerow(['class weight', 'epochs', 'regularization', 'reg. constant',
                    ...
                    'train AUC', 'test AUC'])
        for cls_wt, ep, reg, lam, lr, lyr, drt in grid_params:
            auc_train = []
            auc_test = []
            ...
            print("Train AUC: %.3f, Test AUC: %.3f" % (float(np.mean(auc_train)), float(np.mean(auc_test))))
            w.writerow([str(cls_wt), str(ep), names[reg], str(lam), str(lr), str(lyr), str(drt),
