...
# Run the GCN but first filter the graphs on which we fail to learn.
# There are not many of such. For size 500, we look at clique sizes 20-22.
# The number of graph is taken to be divisible by 5.
class GCNWithoutFailingGraphs:
    good_runs_500 = {20: [1, 2, 3, 4, 5, 6, 11, 14, 15, 18],
                     21: [0, 2, 5, 6, 7, 8, 9, 12, 14, 19],
                     22: [1, 2, 3, 4, 5, 6, 7, 9, 11, 12, 13, 14, 16, 17, 18]}
...
def remaining_vertices_analysis(sizes):
    for sz, cl_sz in sizes:
        ...
        dumping_path = os.path.join("remaining_without_failing", key_name + "_runs")
        ...
        if sz == 500:
            ...
            params = PARAMS_2000
        gcn = GCNWithoutFailingGraphs(sz, PROB, cl_sz, DIRECTED, features=params["features"])
...
def inspect_second_phase(sizes, filename):
    ...
    for sz, cl_sz in sizes:
        print(str(sz) + ",", cl_sz)
        key_name = 'n_' + str(sz) + '_p_' + str(PROB) + '_size_' + str(cl_sz) + ('_d' if DIRECTED else '_ud')
        head_path = os.path.join("remaining_without_failing", key_name + "_runs")
    ...
    measurements_df.to_excel(os.path.join("remaining_without_failing", filename), index=False)
...
def loss_and_roc(sizes):
    for sz, cl_sz in sizes:
        ...
        plt.savefig(os.path.join(os.path.dirname(__file__), 'figures', 'loss_roc_without_failing_%d_%d.png' % (sz, cl_sz)))
...
def is_consistent(sizes, filename):
    ...
    report_df.to_excel(os.path.join("remaining_without_failing", filename), index=False)


# Input distributions
def feature_distributions(sizes, filename):
    ...
    for i, (sz, cl_sz) in enumerate(sizes):
        ...
        if sz == 500:
            ...
            params = PARAMS_2000
        gcn = GCNWithoutFailingGraphs(sz, PROB, cl_sz, DIRECTED, features=params['features'])
        features = gcn.feature_matrices(gcn.good_runs_500[cl_sz])
        clique_vertices = gcn.labels
        ...
        fig, ax = plt.subplots(len(features) + 1, combined_feature_matrix.shape[1], figsize=(12, 8), sharex='col')
...
if __name__ == '__main__':
    ...
    # performance_test_gcn("GCN_without_failing", n_cs)
    # loss_and_roc(n_cs)
    # inspect_second_phase(n_cs, "n_500_cs_20-22_without_failing_run_analysis_v0.xlsx")
    # is_consistent(n_cs, "n_500_cs_20-22_without_failing_remaining_vertices.xlsx")
    feature_distributions(n_cs, "without_failing_feature_distributions")
