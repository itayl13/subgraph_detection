    ...
    with open(filename + '_test.csv', 'w') as f:
        with open(filename + '_train.csv', 'w') as g:
            ...
            for sz, cl_sz in sizes:
                print(str(sz) + ",", cl_sz)
                if sz == 500:
                    gcn = GCNCliqueDetector(sz, 0.5, cl_sz, False, features=['Degree', 'Betweenness', 'BFS'], norm_adj=True)
                    input_params = {
                    ...
                    gcn = GCNCliqueDetector(sz, 0.5, cl_sz, False, features=['Motif_3', 'additional_features'], norm_adj=True)
                    input_params = {
                        "hidden_layers": [55, 325, 325, 320, 195],
                        "epochs": 100,
                        "dropout": 0.45,
                        "lr": 0.03,
                        "regularization": 0.25,
                ...
                test_scores, test_lbs, train_scores, train_lbs = gcn.single_implementation(input_params=input_params, check='CV')
                test_auc = []
        ...
        with open(filename + '_train.csv', 'w') as g:
            ...
            for f_comb in feature_combinations:
                for sz, cl_sz in sizes:
                    ...
                    input_params["class_weights"] = {0: (float(sz) / (sz - cl_sz)) ** input_params["class_weights"],
                                                     1: (float(sz) / cl_sz) ** input_params["class_weights"]}
                    gcn = GCNCliqueDetector(sz, 0.5, cl_sz, False, features=f_comb['features'], norm_adj=True)
...
if __name__ == "__main__":
    ...
    name = 'GCN_vs_DM_many_graphs'
    # name = 'GCN_cheap_vs_expensive'
